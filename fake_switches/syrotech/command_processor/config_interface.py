...
class ConfigInterfaceCommandProcessor(BaseCommandProcessor):
    ...
    #     port_channel_name = f"Port-channel{port_channel_id}"

    #     if not self.port_channel_exists(port_channel_name):
    #         self.write_line(
    #             f"Creating a port-channel interface Port-channel {port_channel_id}"
    #         )
    ...
    #         new_ip = IPNetwork(f"{args[1]}/{args[2]}")
    ...
    #                 self.port.remove_ip(new_ip)
    #                 self.port.ips[0] = new_ip
    #         elif ip_owner.ips.index(existing_ip) == 0:
    ...
    #     if "vrf".startswith(args[0]) and "forwarding".startswith(args[1]):
    #         if isinstance(self.port, VlanPort):
    #             for ip in self.port.ips[:]:
    #                 self.port.remove_ip(ip)
    #         if vrf := self.switch_configuration.get_vrf(args[2]):
    #             self.port.vrf = vrf
    #         else:
    ...
    # TODO Rename this here and in `do_ip`
    # def _extracted_from_do_ip_49(self):
    #     self.write_line(" ^")
    ...
    #     self.write_line(
    #         f"% ip verify configuration not supported on interface Vl{self.port.vlan_id}"
    #     )
    ...
    #     self.write_line(" - verification not supported by hardware")
    #     self.write_line(
    #         f"%Restoring the original configuration failed on {self.port.name} - Interface Support Failure"
    #     )
    ...
    #         vrrp.preempt = True
    #         if args and " ".join(args[:2]) == "delay minimum":
    ...
    #         if "preempt".startswith(command):
    #             if len(args) <= 0 or not "delay".startswith(args[0]):
    #                 vrrp.preempt = None
...
#         if "-" in r:
#             start, stop = r.split("-")
#             vlans += list(range(int(start), int(stop) + 1))
#         else:
#             vlans.append(int(r))
...
#             f"Ip format {strict_format} validation not supported."
#         )
#     try:
#         return IPAddress(ip)
#     except Exception:
